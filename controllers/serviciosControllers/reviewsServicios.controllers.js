...
export const createReview = async (req, res) => {
  const { titulo, comentario, calificacion, id_servicio, id_usuario } = req.body;
  try {
    const newReview = await ReviewServicio.create({ titulo, comentario, calificacion, id_servicio, id_usuario });
    res.json(newReview);
  } catch (error) {
    console.log(error);
...
export const updateReviewById = async (req, res) => {
  ...
  try {
    const [updatedRowsCount, updatedRows] = await ReviewServicio.update({ titulo, comentario, calificacion }, { where: { id }, returning: true });
